...
\vspace{1em}
Optical design, generally speaking, can refer to all activities that involving manipulate the property of light to achieving a certain purpose. However, conventionally, it usually means designing optical systems where light is mainly treated with its geometrical model rather than its more complicated wave model. These optical systems can be a photographic camera, a lamp or a laser beam-shaper. The simplification of the model is mostly driven by the practicality of optical design: with the more rigorous wave model, designing and analysing an common optical system is computationally expensive and hence very time consuming . On the other side, with geometrical optics where light is simplified with rays pointing towards the traveling direction of radiation energy, billion of rays can be traced in seconds for a normal modern computer. The aperture sizes of these optical systems are normally from several millimeters to meters. They are very large compared to the operating wavelength of the systems. Therefore, it is safe to apply the geometrical model and it maintains the essence of light that is sufficient to obtain a reasonable optical system design.  

In some occasions, such as an imaging system close to diffraction limited performance, wave model is necessary to be applied to analyze the system performance, where geometrical model does not help anymore. As a result, common practice for optical system design may involve both geometrical and wave model. Geometrical model is usually used to rapidly obtain a designed system. Wave model helps to provide analysis result which gives feedback to fine-tune the system performance. The later part may only involved in very demanding systems such as microscopic objective or lithographic system. In this thesis, the discussion is limited to the part of geometrical optics. 
